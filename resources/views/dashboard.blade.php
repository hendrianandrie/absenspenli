...
<div class="row g-3 mb-4">
    @if(isset($isGuru) && $isGuru)
        ...
        <!-- FILTER PILIH KELAS & REKAP ABSENSI KELAS GURU -->
        <div class="col-12 mt-3">
            <div class="card card-custom p-4 bg-white shadow-sm mb-4">
                <form method="GET" action="{{ route('dashboard') }}" class="row g-3 align-items-center mb-3">
                    <div class="col-md-6">
                        <label class="form-label fw-bold text-dark fs-6 mb-1">
                            <i class="fa-solid fa-filter text-primary me-2"></i> Pilih Kelas Diampu (Data dari Absen & Siswa)
                        </label>
                        <select name="kelas_guru" class="form-select form-select-lg border-primary fw-bold" onchange="this.form.submit()">
                            @foreach($guruDaftarKelas as $gk)
                                <option value="{{ $gk }}" {{ $selectedGuruKelas == $gk ? 'selected' : '' }}>
                                    Kelas {{ $gk }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 text-md-end pt-md-4">
                        <span class="badge bg-light text-dark border px-3 py-2 fs-6">
                            <i class="fa-solid fa-users text-primary me-1"></i> Total Siswa Kelas {{ $selectedGuruKelas }}: <strong>{{ $guruAbsenStats['total'] }} Siswa</strong>
                        </span>
                    </div>
                </form>

                <!-- Ringkasan Status Absensi Hari Ini di Kelas Terpilih -->
                <div class="row g-2 text-center mb-3">
                    <div class="col-6 col-md-3">
                        <div class="p-2 border rounded bg-success-subtle border-success-subtle">
                            <small class="text-success fw-bold d-block text-uppercase" style="font-size: 11px;">Hadir Hari Ini</small>
                            <span class="fs-5 fw-bold text-success">{{ $guruAbsenStats['hadir'] }} Siswa</span>
                        </div>
                    </div>
                    <div class="col-6 col-md-3">
                        <div class="p-2 border rounded bg-warning-subtle border-warning-subtle">
                            <small class="text-warning-emphasis fw-bold d-block text-uppercase" style="font-size: 11px;">Sakit (S)</small>
                            <span class="fs-5 fw-bold text-warning-emphasis">{{ $guruAbsenStats['sakit'] }} Siswa</span>
                        </div>
                    </div>
                    <div class="col-6 col-md-3">
                        <div class="p-2 border rounded bg-info-subtle border-info-subtle">
                            <small class="text-info fw-bold d-block text-uppercase" style="font-size: 11px;">Izin (I)</small>
                            <span class="fs-5 fw-bold text-info">{{ $guruAbsenStats['izin'] }} Siswa</span>
                        </div>
                    </div>
                    <div class="col-6 col-md-3">
                        <div class="p-2 border rounded bg-danger-subtle border-danger-subtle">
                            <small class="text-danger fw-bold d-block text-uppercase" style="font-size: 11px;">Alpha (A)</small>
                            <span class="fs-5 fw-bold text-danger">{{ $guruAbsenStats['alpha'] }} Siswa</span>
                        </div>
                    </div>
                </div>

                <!-- Tabel Daftar Siswa, Status Absensi & Nilai Akhir Mapel di Kelas Terpilih -->
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <h6 class="fw-bold text-dark mb-0">
                        <i class="fa-solid fa-list-ol me-2 text-primary"></i> Daftar Siswa, Kehadiran Hari Ini & Nilai {{ $guruMapel->nama_mapel ?? '' }} (Kelas {{ $selectedGuruKelas }})
                    </h6>
                    <a href="{{ route('nilai.index', ['kelas' => $selectedGuruKelas, 'mata_pelajaran_id' => $guruMapel->id ?? '']) }}" class="btn btn-sm btn-outline-primary fw-semibold">
                        <i class="fa-solid fa-pen-to-square me-1"></i> Ke Matriks Penilaian Lengkap
                    </a>
                </div>
                    ...
                    <table class="table table-hover align-middle mb-0 text-center border">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th style="width: 140px;">Nilai {{ $guruMapel->kode_mapel ?? 'Mapel' }}</th>
                                <th style="width: 140px;">Status KKM</th>
                                <th style="width: 110px;">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($guruSiswasWithAbsen as $idx => $row)
                                <tr>
                                    <td>{{ $idx + 1 }}</td>
                                    <td class="text-start fw-semibold">{{ $row['siswa']->nama }}</td>
                                    <td>
                                        @if($row['status_absen'] === 'Hadir')
                                            <span class="badge bg-success-subtle text-success border border-success-subtle"><i class="fa-solid fa-circle-check me-1"></i> Hadir</span>
                                        @elseif($row['status_absen'] === 'Sakit')
                                            <span class="badge bg-warning-subtle text-warning-emphasis border border-warning-subtle"><i class="fa-solid fa-briefcase-medical me-1"></i> Sakit</span>
                                        @elseif($row['status_absen'] === 'Izin')
                                            <span class="badge bg-info-subtle text-info border border-info-subtle"><i class="fa-solid fa-envelope me-1"></i> Izin</span>
                                        @else
                                            <span class="badge bg-danger-subtle text-danger border border-danger-subtle"><i class="fa-solid fa-circle-xmark me-1"></i> Alpha</span>
                                        @endif
                                    </td>
                                    <td class="fw-bold fs-6">
                                        @if($row['nilai_akhir'] !== null)
                                            <span class="{{ $row['is_tuntas'] ? 'text-primary' : 'text-danger' }}">{{ $row['nilai_akhir'] }}</span>
                                        @else
                                            <span class="text-muted fs-7">-</span>
                                        @endif
                                    </td>
                                    <td>
                                        @if($row['is_tuntas'] !== null)
                                            @if($row['is_tuntas'])
                                                <span class="badge badge-kkm-pass px-2 py-1 fs-7"><i class="fa-solid fa-check me-1"></i> Tuntas</span>
                                            @else
                                                <span class="badge badge-kkm-fail px-2 py-1 fs-7"><i class="fa-solid fa-triangle-exclamation me-1"></i> Remedial</span>
                                            @endif
                                        @else
                                            <span class="text-muted fs-7">Belum Ada Nilai</span>
                                        @endif
                                    </td>
                                    <td>
                                        <a href="{{ route('nilai.raporSiswaPdf', $row['siswa']->id) }}" class="btn btn-sm btn-outline-danger" title="Cetak Rapor Siswa PDF">
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="text-muted py-3">Belum ada siswa di kelas ini.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
